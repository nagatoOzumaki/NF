...
export default function TableForm({description, setDescription, quantity,
    setQuantity,price ,setPrice ,amount ,setAmount,list,setList, somme, setSomme,Avance,setAvance, somme1, setSomme1})
    {
        ...
        useEffect(()=>{
            ...
            for (let i=0; i < rows.length; i++){
                if(rows[i].className === "text-xl font-bold font-normal text-black amount"){
                    sum1 +=isNaN(rows[i].innerHTML) ? 0 : parseInt(rows[i].innerHTML)
                    setSomme1(sum1)
                }
            }
          })
        // Supposez que 'avance' soit un nombre que vous souhaitez soustraire de la somme.
     // Supposons que 'avance' soit une variable d'état qui peut varier.
...
useEffect(() => {
  ...
  // Soustrayez la valeur actuelle d'avance de la somme obtenue
  const sommeApresSoustraction = sum - Avance;

  // Mettez à jour l'état avec la nouvelle somme
  setSomme(sommeApresSoustraction);
}, [Avance]); // Utilisez 'avance' comme dépendance pour réagir à ses variations
    ...
    return(
        ...
        <thead>
            ...
            <div>
                <h2 className="flex items-end justify-end text-4xl font-bold font-normal text-black">Total HT:{somme1.toLocaleString()}Dh&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</h2>
            </div>
            <div>
                <h2 className="flex items-end justify-end text-4xl font-bold font-normal text-black">Reste:{somme.toLocaleString()}Dh&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</h2>
            </div>
